...
if(!isset($_GET['id']) || !is_numeric($_GET['id'])) {
    ...
    $query = "SELECT id_ricetta, nome, descrizione, unita_misura, ultima_modifica, id_utente,tempo_preparazione,tempo_cottura FROM ricette WHERE (id_ingrediente = ?)";
    ...
    if($statement->execute()) {
        ...
        if($statement->num_rows == 0) {
            ...
            $statement->bind_result($idricetta, $nome, $descrizione, $unitaMisura, $ultimaModifica, $idUtente, $tempoPreparazione, $tempoCottura); // Abbina i risultati della query alle variabili
...
if(isset($_POST['addBtn'])) {
    ...
    $query = "UPDATE ricette SET nome = ?, descrizione = ?, unita_misura = ?, ultima_modifica = ?, tempo_cottura=?, tempo_preparazione=? WHERE id_ricetta = ?";
    $statement = $mysqli->prepare($query);
    $statement->bind_param('ssssssi', $nome, $descrizione, $unitaMisura, $modificaTempo,$tempoCottura,$tempoPreparazione, $idricetta);
    // Esegui la query
    if($statement->execute()) {
        $messaggio = '<div class="alert alert-success mt-3" role="alert">Ingrediente modificato con successo. <a href="visualizza-ricetta.php?id=' . $idricetta . '">Visualizza ricette</a>.</div>';
